...
使用基尼指数进行划分选择, 未剪枝的决策树算法:

```bash
(xiguashu)  ~/workspace/xiguashu $ python3 -m decision_tree.gini
输入-数据集 D:
...
输入-属性集 A:
{Discrete attribute: name=脐部, values={'平坦', '稍凹', '凹陷'}, Discrete attribute: name=触感, values={'硬滑', '软粘'}, Discrete attribute: name=根蒂, values={'硬挺', '稍蜷', '蜷缩'}, Discrete attribute: name=敲声, values={'浊响', '清脆', '沉闷'}, Discrete attribute: name=纹理, values={'模糊', '清晰', '稍糊'}, Discrete attribute: name=色泽, values={'浅白', '乌黑', '青绿'}}

输出-决策树:
Classify(分类属性): 脐部, children(3):
|--脐部=平坦: Label(标记值): 否
|--脐部=稍凹: Classify(分类属性): 根蒂, children(3):
  |--根蒂=硬挺: Label(标记值): 是
  |--根蒂=稍蜷: Classify(分类属性): 纹理, children(3):
    |--纹理=模糊: Label(标记值): 是
    |--纹理=清晰: Classify(分类属性): 色泽, children(3):
      |--色泽=浅白: Label(标记值): 是
      |--色泽=乌黑: Label(标记值): 否
      |--色泽=青绿: Label(标记值): 是
    |--纹理=稍糊: Label(标记值): 是
  |--根蒂=蜷缩: Label(标记值): 否
|--脐部=凹陷: Classify(分类属性): 根蒂, children(3):
  |--根蒂=硬挺: Label(标记值): 是
  |--根蒂=稍蜷: Label(标记值): 否
  |--根蒂=蜷缩: Label(标记值): 是
==========
...
输入-属性集 A:
{Discrete attribute: name=根蒂, values={'稍蜷', '蜷缩', '硬挺'}, Discrete attribute: name=敲声, values={'浊响', '清脆', '沉闷'}, Discrete attribute: name=纹理, values={'模糊', '稍糊', '清晰'}, Discrete attribute: name=脐部, values={'凹陷', '稍凹', '平坦'}, Discrete attribute: name=触感, values={'软粘', '硬滑'}, Discrete attribute: name=色泽, values={'乌黑', '青绿', '浅白'}}
