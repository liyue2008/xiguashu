...
def back_propagation(nn: NeualNetworks, training_set: TrainingSet, learning_rate: float, stop_function: Callable[[TrainingSet, NeualNetworks, Any, Dict[str, Any]], Tuple[bool, Any]], config: Dict[str, Any] = {}) -> None:
    ...
    while True:
        ...
        for index, sample in training_set.samples.iterrows():
            ...
            for layer_index in reversed(range(len(nn.layers) - 1)): # 自顶向下遍历所有的隐层和输入层，计算每一隐层神经元的梯度项
                ...
                gradient_current_layer = __calc_layer_gradient(nn, layer_index, output_current_layer, gradient_upper_layer) # 本层神经元梯度
    ...
    print('当前网络:')
    print(nn)
def __calc_layer_gradient(nn: NeualNetworks,layer_index: int, output_current_layer: List[float], gradient_upper_layer: List[float]) -> List[float]:
